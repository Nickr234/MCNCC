...
folder = 'datasets/FID-300'

parser = argparse.ArgumentParser(description='take some indiviudal folders from user')
parser.add_argument('-t', '--tracks', type = str, default='tracks_cropped', help='define track folder')
parser.add_argument('-rf', '--refs', type=str, default='references', help='define reference folder')
...
parser.add_argument('-lbltable', '--label_file', type=str, default='datasets/fid-300/label_table.csv',help='name of the csv. file')
...
if args.rot == False:
    ...
    for x, t in enumerate(tqdm(np.sort(track_l))):
        ...
        template = model(trans(template).unsqueeze(0).to(device))[0]
        template_t2 = template[:, 3:template.shape[1]-3, 3:template.shape[2]-3]
        ncc = NCC(template_t2)
        
        for y, r in enumerate(np.sort(ref_l)):
            torch.cuda.empty_cache()
            image = Image.open(path.join(refs, r))
            image = model(trans(image).unsqueeze(0).to(device))
            image_t2 = image[:,:,3:image.shape[2]-3, 3:image.shape[3]-3].to(device)
