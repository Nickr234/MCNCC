...
parser.add_argument('-t', '--tracks', type = str, default='tracks_cropped_Subset_NP', help='define track folder')
parser.add_argument('-rf', '--refs', type=str, default='Ref_Sub_NP', help='define reference folder')
...
track_l = [f for f in os.listdir(tracks) if f.endswith('.png')]
...
if args.rot == False:
    ...
    for x, t in enumerate(tqdm(np.sort(track_l))):
        ...
        template = template[:, 3:template.shape[1]-3, 3:template.shape[2]-3]
        ncc = NCC(template)
        ...
        for y, r in enumerate(np.sort(ref_l)):
            ...
            torch.cuda.empty_cache()
            image = image[:,:,3:image.shape[2]-3, 3:image.shape[3]-3].to(device)

            ncc_response = ncc(image)
